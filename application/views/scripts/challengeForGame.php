<div class="span9 contentBG">
    <h2>Challenge for game player: <?php echo $user_data['name']; ?></h2>
    ...
    <table width="100%" border="0">
        <tr>
           <td style="width: 280px; ">
                <?php 
                    $destination = PROFILE_IMAGE.DIRECTORY_SEPARATOR."{$user['user_id']}.png"; 
                    $picName = file_exists($destination) ? "{$user['user_id']}.png" : 'profile_image.png'; 
                ?>
                <img src='<?php echo base_url()."www/img/profile/$picName"; ?>' width="120" /></a>
               &nbsp;VS&nbsp;
                <?php 
                    $destination = PROFILE_IMAGE.DIRECTORY_SEPARATOR."{$user_data['user_id']}.png"; 
                    $picName = file_exists($destination) ? "{$user_data['user_id']}.png" : 'profile_image.png'; 
                ?>
                <img src='<?php echo base_url()."www/img/profile/$picName"; ?>' width="120" /> 
               <br /><br />
            </td>
            <td> 
                <form action="<?php echo base_url(); ?>challengeForGame/<?php echo $user_data['user_id']; ?>" method="POST">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="form-actions">
                                    <input type="hidden" name="user_id1" value="<?php echo $user['user_id']; ?>">
                                    <input type="hidden" name="user_id2" value="<?php echo $user_data['user_id']; ?>">
                                    <input type="submit" value="Challenge for game" class="btn btn-primary" />
                                </div>
                            </td>
                        </tr>
                    </tbody>
